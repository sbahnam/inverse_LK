...
cv::Mat ComputeImageGradient(const cv::Mat& _image) {
      ...
      for ( j = 0; j < imCols-1; ++j)
        {
          ...
          if (j > 0)
            ...
            p[k*gradCols] = q[k + i + 1] - q[k + i]; //@todo: add k++ in i loop and remove + i in q[]
  ...
  // else
  // {
  //   std::cout<<"WARNING::ComputeImageGradient:: cv mat not aligned, this decrease computational performance"<<std::endl;
...
cv::Mat ComputeInvHessian(const cv::Mat& m_j) {
...
float ComputeResiduals(cv::Mat& m_residuals,
                                  ...
                                  const int& level)
{
  ...
  // else
  // {
  //   std::cout<<"WARNING::ComputeResiduals:: cv mat not aligned, this decrease computational performance"<<std::endl;
...
cv::Mat ComputeUpdateParams(const cv::Mat& m_inv_hessian, 
                                        const cv::Mat& m_J,
  ...
  // else
  // {
  //   std::cout<<"WARNING::ComputeUpdateParams:: cv mat not aligned, this decrease computational performance"<<std::endl;
  //   for(int idx = 0; idx < m_residuals.rows * m_residuals.cols; idx++) {
  //     dp1 += m_J.at<float>(idx,0) * m_residuals.at<float>(idx,0); //@todo: ComputeUpdateParams and ComputeResiduals can be
...
bool IsConverged(const float& current_error, const float& min_error, //@todo: add maximum distance  "a tracker inside a search region"
                      const float epsilon, const float dxy, const int level)
{
...
void ProcessInLayer(cv::Point2f &delta, const cv::Point2f& x0y0,
                                float& error,
                                const int& max_iter,
                                const float& epsilon,
                                ...
                                const cv::Mat& m_J)
{
  ...
  static cv::Mat1f m_update_params = cv::Mat::zeros(2,1, CV_32F); // @todo as input/output to ComputeUpdateParams & chagne to cvPOint2f
  static float dx_i;
  static float dy_i;
  ...
  errors_lst.clear();
  delta_xys.clear();
  float prev_dxy = 9999.f;

  // Inverse Compositional Algorithm
  for(int itr = 0; itr < max_iter; itr++)
  {
    success_warp = WarpCurrentImage(m_cur_image_in_pyramid, m_cur_working_image, delta.x, delta.y, x0y0, winsize);
    if (success_warp != true) break; // This happens if part of the window goes outside of the image (no border used)
    error = ComputeResiduals(m_residuals, m_cur_working_image, m_ref_image_in_pyramid, level);

    errors_lst.push_back(error);
    delta_xys.push_back(delta);
    // fprintf(stderr,"Itr[%d|%d]: %.6f\n",itr,level,error);
    m_update_params = ComputeUpdateParams(m_inv_hessian, m_J, m_residuals);
    
    delta.x -= m_update_params.at<float>(0);
    delta.y -= m_update_params.at<float>(1);
...
void TrackLKInv(const std::vector<cv::Mat>& vm_image_pyramid, const std::vector<cv::Mat>& vm_current_image_pyramid,
                      ...
                      const cv::Size winsize, const int max_iter, const float epsilon, const float max_distance)
{
  ...
  for (int pt=0; pt<prev_points.size(); pt++)
  {
    ...
    for (int l = 0; l < m_pyramid_level; l++)
    {
                                                                                        ...
                                                                                        // @todo: it should be +1!?
      {
        ...
        mvm_ref_image_pyramid.push_back(warp_interpolater(vm_image_pyramid[l],
      ...
      mvm_J.push_back(ComputeImageGradient(mvm_ref_image_pyramid[l]));
      mvm_inv_hessian.push_back(ComputeInvHessian(mvm_J[l]));
    }
    delta.x=0.f;
    delta.y=0.f;
    ...
    for (int level = 0; level < m_pyramid_level; level++) {
      ...
      ProcessInLayer(delta, mm_H0s[m_pyramid_level - level -1], error, max_iter, epsilon, winsize,
    ...
    inlier_markers.push_back(error < error_threshold &&  delta.x * delta.x + delta.y *delta.y < max_distance);
  }
  return ;
}


bool WarpCurrentImage(const cv::Mat& src, cv::Mat &dst, const float& dx_i, const float& dy_i,
                        const cv::Point2f& x0y0, const cv::Size& winsize)
{
        ...
        x0y0.y +  dy_i + 1 + winsize.height <  src.rows) // rounding to nearest neigbour
  {
    ...
    // else
    // {
    //   std::cout<<"WARNING::WarpCurrentImage:: cv mat not aligned, this decrease computational performance"<<std::endl;
...
//@todo: make warp interpolater a void, create warp vector outside this function and use them as input/ouput
cv::Mat warp_interpolater(const cv::Mat& src, const float& x0, const float& y0, const cv::Size& winsize)
{
  ...
  // else
  // {
  //   std::cout<<"WARNING::warp_interpolater:: cv mat not aligned, this decrease computational performance"<<std::endl;
