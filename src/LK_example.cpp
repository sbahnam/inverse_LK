...
int main(int argc, char* argv[]){
  ...
  prev_cam0_pyramid_restore = prev_cam0_pyramid_;
  curr_cam0_pyramid_restore = curr_cam0_pyramid_;
  ...
  std::vector<cv::Point2f> curr_cam0_points_0s;
  std::vector<cv::Point2f> prev_cam0_points_0s;
  cv::Point2f p0 = cv::Point2f(30, 30);
  curr_cam0_points_0s.push_back(p0);
  prev_cam0_points_0s.push_back(p0);


  std::vector<unsigned char> track_inliers(0);
  TrackLKInv(prev_cam0_pyramid_, curr_cam0_pyramid_, // Pyramid images
        prev_cam0_points_0s, curr_cam0_points_0s, // Previous feature location & estimated new location
        track_inliers, 30,
        cv::Size(15, 15), // window size
        30, 0.01, // Stop criteria
        9998); // max distance squared to be inlie
